...
function Shell({ children }: { children: ReactNode }) {
  const { t } = useI18n();
  return <PatientProvider><a href="#main" className="skip-link">{t('skip')}</a><header><div className="wrap header-inner"><Link className="brand" href="/" aria-label={`Exacta7, ${t('home')}`}><span className="brand-symbol" aria-hidden>≋</span>EXACTA<span>7</span></Link><nav aria-label={t('navLabel')}><Link href="/medicamentos">{t('navMedicines')}</Link><Link href="/calculadoras">{t('navTools')}</Link><Link href="/fuentes">{t('navSources')}</Link><Link href="/sobre-exacta7">{t('navAbout')}</Link></nav><LanguageSelector/><ClinicalSearch compact /></div></header><PatientStrip/><main id="main" className="wrap main">{children}</main><footer className="site-footer wrap"><span className="brand small">EXACTA<span>7</span></span><p>Better decisions. Healthier animals.</p><Link href="/fuentes">{t('footerMethod')}</Link><Link href="/sobre-exacta7">{t('navAbout')}</Link><p className="footer-credit">{t('footerCredit')} <a href="https://dentromarketing.es" target="_blank" rel="noopener">Dentro Marketing</a> · {t('footerCreditTail')}</p><span className="muted">{t('footerStatus')}</span></footer></PatientProvider>;
}
