...
export function ClinicalSearch({ compact = false, medicationOnly = false }: { compact?: boolean; medicationOnly?: boolean }) {
  ...
  const featured = useMemo(() => catalog.filter(entry => !filter || entry.group === filter).filter(entry => !entry.id.startsWith('aemps-')).slice(0, 6), [filter]);
  useEffect(() => {
    if (!compact) return;
  ...
  return <div ref={area} className={compact ? 'global-search' : 'search-workspace'} onKeyDown={keys} onBlur={e => { if (compact && !e.currentTarget.contains(e.relatedTarget)) setOpen(false); }}>
    ...
    {show && <div className={compact ? 'search-popover' : ''} id={`${id}-results`}>
      {!compact && !medicationOnly && <div className="filter-tabs" aria-label={t('all')}><button type="button" aria-pressed={!filter} onClick={() => setFilter(undefined)}>{t('all')}</button>{groups.map(group => <button type="button" aria-pressed={filter === group} key={group} onClick={() => setFilter(group)}>{t(group === 'MEDICAMENTOS' ? 'groupMedicines' : group === 'INDICACIONES' ? 'groupIndications' : group === 'CALCULADORAS' ? 'groupCalculators' : group === 'HERRAMIENTAS' ? 'groupTools' : 'groupReferences')}</button>)}</div>}
      {!query.trim() ? <div className="search-idle"><span className="eyebrow">{compact ? t('searchContext') : t('explore')}</span><p>{t('searchHelp')}</p><div className="suggestions">{['Propofol', 'CRI', 'Perro', 'AEMPS'].map(q => <button type="button" key={q} onClick={() => { setQuery(q); input.current?.focus(); }}>{q} <span aria-hidden>↗</span></button>)}</div>{!compact && !medicationOnly && <section className="result-group featured-results" aria-label={filter ? t(filter === 'MEDICAMENTOS' ? 'groupMedicines' : filter === 'INDICACIONES' ? 'groupIndications' : filter === 'CALCULADORAS' ? 'groupCalculators' : filter === 'HERRAMIENTAS' ? 'groupTools' : 'groupReferences') : t('all')}><h3 className="eyebrow">{filter ? t(filter === 'MEDICAMENTOS' ? 'groupMedicines' : filter === 'INDICACIONES' ? 'groupIndications' : filter === 'CALCULADORAS' ? 'groupCalculators' : filter === 'HERRAMIENTAS' ? 'groupTools' : 'groupReferences') : t('all')}</h3><ul>{featured.map(entry => <li key={entry.id}><Link data-result href={entry.href} className="result-link"><span><strong>{entry.title}</strong><small>{entry.description}</small></span><span className="arrow" aria-hidden>→</span></Link></li>)}</ul></section>}</div> : <>
